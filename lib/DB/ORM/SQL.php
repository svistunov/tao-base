...
///     <body>
  public function set() {
    $this->parts['columns'] = array();
    foreach (Core::normalize_args(func_get_args()) as $column)
      $this->parts['columns'][] = (string) $column;
    return $this;
  }
///     </body>
///   </method>
...
///     <body>
  public function as_string() {
    $set = '';
    foreach ($this->parts['columns'] as $v)
      $set .= ($set ? ', ' : '')."$v = :$v";
    $where = '';
    if (isset($this->parts['where'])) $where = implode(') AND (', $this->parts['where']);

    return 'UPDATE '.(isset($this->parts['mode']) ? $this->parts['mode'] : '').' '.$this->parts['table']."\n".
           "SET $set\n".
           ($where ? "WHERE ($where)" : '')."\n";
  }
///     </body>
  ...
  public function columns() {
    ...
    foreach (Core::normalize_args(func_get_args()) as $k => $v) {
      if (is_array($v))
        ...
        if (is_int($k)) $this->parts['columns'][preg_match('{[0-9a-zA-Z_]+\.([0-9a-zA-Z_]+)}', $v, $m) ? $m[1] : $v] = $v;
...
///     <body>
  public function as_string() {
    ...
    $from = '';
    foreach ($this->parts['from'] as $k => $v) $from .= ($from ? ', ' : '').$v.(is_int($k) ? '' : " $k");
    ...
    if (isset($this->parts['joins'])) {
      foreach ($this->parts['joins'] as $join)
        $joins .= strtoupper($join[0]).' JOIN '.$join[1].' ON ('.implode(') AND (', $join[2]).') ';
    }
