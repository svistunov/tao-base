...
class Templates_HTML_Forms implements Core_ModuleInterface, Templates_HelperInterface {
///   <constants>
  const VERSION = '0.2.1';
///   </constants>
...
///     </args>
///     <body>
  public function begin_form(Templates_HTML_Template $t, Forms_Form $form, array $attrs = array()) {
    $this->form = $form;

    $method = Net_HTTP::method_name_for($form->options['method']);

    $result = $t->tag('form', array(
        'action'  => $form->action,
        'method'  => ($method == 'post' || $method == 'get' ? $method : 'post'),
        'id'      => "{$form->name}_form",
        'enctype' => $form->options['enctype']
        ), false).
      $this->begin_fieldset($t, $attrs);
...
///     <body>
  public function select(Templates_HTML_Template $t, $name,  array $attributes = array()) {
    return $this->select_tag($t, $this->field_name_for($name),
      $this->form->fields[$name]->items, $this->form->fields[$name]->value, $attributes);
  }
///     </body>
///   </method>
...
///     <body>
  public function object_multicheckbox(Templates_HTML_Template $t, $name, array $attributes = array()) {
    ...
    foreach ($field->items as $k => $item) {
      ...
      $r .= $this->label($t, '', "$checkbox&nbsp;{$item->$attribute}", array('class' => 'inline'));
    }
