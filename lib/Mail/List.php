...
///     <body>
  protected function body() {
    ...
    IO_FS::File(sprintf('%s/%s.body', $messages_path, $this->id))->
        open('w')->
...
///     <body>
  protected function head($container, $index) {
    ...
    foreach ($container as $k => $v) {
      ...
      IO_FS::File(sprintf('%s/%s.%06d', Mail_List::option('root').'/recipients', $this->id, $index))->
        open('w')->
...
///     <body>
  protected function encoder_for(Mail_Message_Part $msg) {
     return ($msg->head['Content-Transfer-Encoding'] == MIME::ENCODING_QP) ?
