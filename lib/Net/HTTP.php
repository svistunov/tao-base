...
///     </args>
///     <body>
  static public function Upload($tmp_path, $original_name) { return new Net_HTTP_Upload($tmp_path, $original_name); }
///     </body>
///   </method>
...
///     <body>
 private function canonicalize($name) {
    $parts = array_map(create_function('$x', 'return strtolower($x);'), explode('-', trim( str_replace('_', '-', $name))));
...
///     </args>
///     <body>
  public function session(Net_HTTP_SessionInterface $session) {
  ...
  protected function compose_uri() {
    $query = $this->urlencode(
      $this->method == Net_HTTP::GET ?
        ...
        $this->query);
    return $this->uri['scheme'].'://'.$this->uri['host'].$this->uri['path'].($query ? '?'.$query : '');
  }
///     </body>
///   </method>
...
///     </args>
///     <body>
  public function __construct($tmp_path, $original_name) {
