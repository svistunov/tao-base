...
///     <body>
  public function append($object, $index = null) {
    if (is_null($index)) {
      $this->delegates[]   = $object;
      $this->reflections[] = new ReflectionObject($object);
    } else {
      $this->delegates[$index]   = $object;
      $this->reflections[$index] = $object;
    }
...
///     <body>
  public function getIterator() { return new ArrayIterator($this->delegates); }
///     </body>
///   </method>
...
///     </args>
///     <body>
  public function offsetExists($index) { return isset($this->delegates[$index]); }
///     </body>
///   </method>
...
///     </args>
///     <body>
  public function offsetUnset($index) { unset($this->delegates[$index]); }
///     </body>
///   </method>
...
///     <body>
  public function __call($method, $args) {
    if (!isset($this->methods[$method])) {
      foreach ($this->reflections as $k => $r)
        if ($r->hasMethod($method)) {
          $this->methods[$method] = array($this->delegates[$k], $r->getMethod($method));
          break;
        }
    }
    switch (true) {
  ...
  public function map($name, $type = null) {
    switch (true) {
      case is_array($name):
        ...
        foreach ($name as $k => $v) $this->map[$k] = Core_Types::reflection_for("$prefix$v");
        break;
      case is_string($name):
        if ($type)
...
///     <body>
  public function new_instance_of($name, $args = array()) {
    if (isset($this->map[$name]))
      return count($args) ?
        $this->map[$name]->newInstanceArgs($args) :
        $this->map[$name]->newInstance();
    else
